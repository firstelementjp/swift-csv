...
class FE_CSV_Import_Export_Ajax_Import_Unified {
	 ...
	 *
	 * @since 0.9.8
	 * @var FE_CSV_Import_Export_Import_Log_Store|null
	 */
	private $log_store;
	 ...
	 *
	 * @since 0.9.8
	 * @var FE_CSV_Import_Export_Import_Request_Parser|null
	 */
	...
	public function __construct( bool $register_hooks = true ) {
		if ( $register_hooks ) {
			add_action( 'wp_ajax_fe_csv_import_export_ajax_import', [ $this, 'handle' ] );
			add_action( 'wp_ajax_fe_csv_import_export_ajax_import_logs', [ $this, 'handle_ajax_import_logs' ] );
			add_action( 'wp_ajax_fe_csv_import_export_cancel_import', [ $this, 'handle_ajax_import_cancel' ] );
		}
	}
	 ...
	 *
	 * @since 0.9.8
	 * @return FE_CSV_Import_Export_Import_Request_Parser
	 */
	private function get_request_parser(): FE_CSV_Import_Export_Import_Request_Parser {
		if ( null === $this->request_parser ) {
			$this->request_parser = new FE_CSV_Import_Export_Import_Request_Parser();
		}
	...
	public function handle_ajax_import_logs(): void {
		...
		FE_CSV_Import_Export_Ajax_Util::set_initial_ob_level( $initial_ob_level );
		FE_CSV_Import_Export_Ajax_Util::set_ajax_action( 'fe_csv_import_export_ajax_import_logs' );
		FE_CSV_Import_Export_Ajax_Util::register_fatal_error_json_handler();
		FE_CSV_Import_Export_Ajax_Util::register_empty_response_json_handler();

		check_ajax_referer( 'fe_csv_import_export_ajax_nonce', 'nonce' );

		$can_import = (bool) apply_filters( 'fe_csv_import_export_user_can_import', (bool) current_user_can( 'import' ) );
		if ( ! $can_import ) {
		...
		$precheck_result = apply_filters( 'fe_csv_import_export_pre_ajax_import', true, $sanitized_post );
	 ...
	 *
	 * @since 0.9.8
	 * @return FE_CSV_Import_Export_Import_Log_Store
	 */
	private function get_log_store(): FE_CSV_Import_Export_Import_Log_Store {
		if ( null === $this->log_store ) {
			$this->log_store = new FE_CSV_Import_Export_Import_Log_Store();
		}
	...
	public function handle(): void {
		...
		FE_CSV_Import_Export_Ajax_Util::set_initial_ob_level( $initial_ob_level );
		FE_CSV_Import_Export_Ajax_Util::set_ajax_action( 'fe_csv_import_export_ajax_import' );
		FE_CSV_Import_Export_Ajax_Util::register_fatal_error_json_handler();
		FE_CSV_Import_Export_Ajax_Util::register_empty_response_json_handler();
	...
	private function handle_direct_sql_import(): void {
		$handler = new FE_CSV_Import_Export_Ajax_Import_Handler_Direct_SQL();
		$handler->handle();
	}
	...
	private function handle_wp_compatible_import(): void {
		$handler = new FE_CSV_Import_Export_Ajax_Import_Handler_WP_Compatible();
		$handler->handle();
	}
	...
	private function is_direct_sql_import_enabled(): bool {
		return (bool) apply_filters( 'fe_csv_import_export_enable_direct_sql_import', false );
	}
}
