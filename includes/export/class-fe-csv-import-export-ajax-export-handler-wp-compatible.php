...
class FE_CSV_Import_Export_Ajax_Export_Handler_WP_Compatible {
	 ...
	 *
	 * @since 0.9.8
	 * @var FE_CSV_Import_Export_Export_Log_Store
	 */
	private $log_store;
	 ...
	 *
	 * @since 0.9.8
	 * @var FE_CSV_Import_Export_Export_Cancel_Manager
	 */
	 ...
	 *
	 * @since 0.9.8
	 * @var FE_CSV_Import_Export_Ajax_Export_Batch_Planner
	 */
	 ...
	 *
	 * @since 0.9.8
	 * @param FE_CSV_Import_Export_Export_Log_Store      $log_store Export log store.
	 * @param FE_CSV_Import_Export_Export_Cancel_Manager $cancel_manager Export cancel manager.
	 */
	public function __construct( FE_CSV_Import_Export_Export_Log_Store $log_store, FE_CSV_Import_Export_Export_Cancel_Manager $cancel_manager ) {
		$this->log_store      = $log_store;
		$this->cancel_manager = $cancel_manager;
		$this->batch_planner  = new FE_CSV_Import_Export_Ajax_Export_Batch_Planner();
	}
	...
	public function handle( array $config ): array {
		...
		try {
			...
			if ( 0 === $start_row ) {
				$headers_key  = 'fe_csv_import_export_csv_headers_' . get_current_user_id() . '_' . $export_session;
				$headers_line = get_transient( $headers_key );
				if ( ! is_string( $headers_line ) || '' === $headers_line ) {
					$export       = new FE_CSV_Import_Export_Export_WP_Compatible( $config );
			...
			$transient_key = 'fe_csv_import_export_unified_export_config_' . get_current_user_id() . '_' . $export_session;
			...
			$export     = isset( $export ) && $export instanceof FE_CSV_Import_Export_Export_WP_Compatible ? $export : new FE_CSV_Import_Export_Export_WP_Compatible( $config );
			...
			if ( isset( $_POST['enable_logs'] ) && in_array( (string) $_POST['enable_logs'], [ '1', 'true' ], true ) ) {
				...
				/* translators: 1: Batch number, 2: Number of posts processed */
					__( 'Batch %1$d: Bulk export %2$d posts', 'fe-csv-import-export' ),
					$batch_number,
					count( $posts_data )
				);
