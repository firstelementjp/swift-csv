<?php
/**
 * Post persistence for FE CSV Import & Export import.
 *
 * @since 0.9.0
 ...
 * @package FE_CSV_Import_Export
 */
class FE_CSV_Import_Export_Import_Persister {
	/**
	...
	public function execute_post_update( $wpdb, $post_id, $post_data, $dry_run, &$dry_run_log ) {
		...
		if ( $dry_run ) {
			...
			$dry_run_log[]     = sprintf(
				...
				__( 'Update post: ID=%1$s, title=%2$s', 'fe-csv-import-export' ),
				$post_id,
	...
	public function execute_post_insert( $wpdb, $post_data, $dry_run, &$dry_run_log, &$post_id ) {
		...
		if ( $dry_run ) {
			...
			$dry_run_log[]     = sprintf(
				...
				__( 'New post: title=%1$s', 'fe-csv-import-export' ),
				$post_data['post_title'] ?? 'Untitled'
			);
			if ( count( $dry_run_log ) > $dry_run_log_limit ) {
				$dry_run_log = array_slice( $dry_run_log, -1 * $dry_run_log_limit );
