...
class FE_CSV_Import_Export_Ajax_Export_Unified {
	...
	public function __construct() {
		...
		if ( class_exists( 'FE_CSV_Import_Export_Export_Log_Store' ) ) {
			$this->log_store = new FE_CSV_Import_Export_Export_Log_Store();
		}
		$this->cancel_manager = new FE_CSV_Import_Export_Export_Cancel_Manager();

		// Enable unified handler for Direct SQL export.
		add_action( 'wp_ajax_fe_csv_import_export_ajax_export', [ $this, 'handle_ajax_export' ] );
		add_action( 'wp_ajax_fe_csv_import_export_ajax_export_logs', [ $this, 'handle_ajax_export_logs' ] );
		add_action( 'wp_ajax_fe_csv_import_export_cancel_export', [ $this, 'handle_ajax_export_cancel' ] );
	}
	...
	public function handle_ajax_export() {
		...
		if ( ! wp_verify_nonce( $nonce, 'fe_csv_import_export_ajax_nonce' ) ) {
		...
		$can_export = (bool) apply_filters( 'fe_csv_import_export_user_can_export', (bool) current_user_can( 'export' ) );
		if ( ! $can_export ) {
			while ( function_exists( 'ob_get_level' ) && function_exists( 'ob_end_clean' ) && ob_get_level() > $initial_ob_level ) {
				ob_end_clean();
			}
		...
		$precheck_result = apply_filters( 'fe_csv_import_export_pre_ajax_export', true, $sanitized_post );
	...
	private function check_rate_limit() {
		...
		$cache_key    = 'fe_csv_import_export_concurrent_export_' . $session_id;
	...
	private function handle_direct_sql_export( $config ) {
		$handler_class = 'FE_CSV_Import_Export_Ajax_Export_Handler_Direct_SQL';

		if ( class_exists( 'FE_CSV_Import_Export_Pro_Ajax_Export_Handler_Direct_SQL' ) ) {
			$handler_class = 'FE_CSV_Import_Export_Pro_Ajax_Export_Handler_Direct_SQL';
		}
	...
	private function handle_wp_compatible_export( $config ) {
		$handler = new FE_CSV_Import_Export_Ajax_Export_Handler_WP_Compatible( $this->log_store, $this->cancel_manager );
		return $handler->handle( (array) $config );
	}
	...
	private function is_direct_sql_export_enabled(): bool {
		...
		$has_pro_plugin        = class_exists( 'FE_CSV_Import_Export_Pro_Admin' )
			|| class_exists( 'FE_CSV_Import_Export_Pro_Ajax_Export_Handler_Direct_SQL' );
