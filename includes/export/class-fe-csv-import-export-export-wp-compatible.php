...
class FE_CSV_Import_Export_Export_WP_Compatible extends \FE_CSV_Import_Export_Export_Base {
	...
	public function wp_compatible_batch_fetch_posts( int $offset, int $batch_size ): array {
		 ...
		 */
		do_action( 'fe_csv_import_export_export_batch_prepare', $post_ids, $posts, $headers, $this->config, 'wp_compatible' );
		...
		foreach ( (array) $posts as $post ) {
			...
			if ( ! empty( $this->config['include_custom_fields'] ) ) {
				...
				foreach ( $custom_field_headers as $header ) {
					...
					$row[ $header ] = \FE_CSV_Import_Export_Helper::join_pipe_separated_values( $values );
				}
			}
			...
			if ( ! empty( $this->config['include_taxonomies'] ) ) {
				foreach ( $taxonomy_names as $taxonomy ) {
					...
					$row[ 'tax_' . $taxonomy ] = \FE_CSV_Import_Export_Helper::join_pipe_separated_values( $values );
				}
			}

			$row    = apply_filters( 'fe_csv_import_export_export_row', $row, $post_id, $this->config, 'wp_compatible' );
			$rows[] = $row;
		}
