...
class FE_CSV_Import_Export_Import_File_Processor {
	...
	public function handle_upload(): ?array {
		...
		if ( ! FE_CSV_Import_Export_Ajax_Util::verify_nonce( $nonce ) ) {
			FE_CSV_Import_Export_Ajax_Util::send_security_error();
			return null;
		}
		...
		if ( ! isset( $_FILES['csv_file'] ) ) {
			...
			if ( $start_row > 0 ) {
				...
				$temp_dir = FE_CSV_Import_Export_File_Util::create_temp_directory();
				...
				FE_CSV_Import_Export_Ajax_Util::send_error_response( 'No existing file found for batch processing' );
				return null;
			} else {
				FE_CSV_Import_Export_Ajax_Util::send_error_response( 'No file uploaded' );
				return null;
			}
		}
		...
		if ( isset( $uploaded_file['error'] ) ) {
			FE_CSV_Import_Export_Ajax_Util::send_error_response( 'Upload error: ' . $uploaded_file['error'] );
			return null;
		}

		// Create temp directory and file path.
		$temp_dir  = FE_CSV_Import_Export_File_Util::create_temp_directory();
		$temp_file = FE_CSV_Import_Export_File_Util::generate_temp_file_path( $temp_dir );

		// Cleanup old temporary files only on first upload.
		$start_row = isset( $_POST['start_row'] ) ? intval( $_POST['start_row'] ) : 0;
		if ( 0 === $start_row ) {
			FE_CSV_Import_Export_File_Util::cleanup_old_temp_files( $temp_dir, $temp_file );
		}

		// Copy uploaded file to temp location.
		if ( ! copy( $uploaded_file['file'], $temp_file ) ) {
			FE_CSV_Import_Export_Ajax_Util::send_error_response( 'Failed to save file' );
