...
class FE_CSV_Import_Export_Import_Csv_Parser {
	 ...
	 */
	public function __construct( ?FE_CSV_Import_Export_Import_Taxonomy_Util $taxonomy_util = null ) {
	...
	public function parse_and_validate_csv_file( string $file_path, array $config ): ?array {
		FE_CSV_Import_Export_Ajax_Util::set_stage( 'csv_parser:parse_and_validate_csv' );

		try {
			// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
			$handle = fopen( $file_path, 'rb' );
			if ( false === $handle ) {
				FE_CSV_Import_Export_Ajax_Util::send_error_response( 'CSV parsing failed: unable to open file' );
				return null;
			}
			...
			if ( null === $first_line ) {
				...
				fclose( $handle );
				FE_CSV_Import_Export_Ajax_Util::send_error_response( 'CSV parsing failed: empty file' );
				return null;
			}
			...
			if ( '' === $taxonomy_format ) {
				// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose
				fclose( $handle );
				FE_CSV_Import_Export_Ajax_Util::send_error_response( 'Missing taxonomy_format' );
				return null;
			}
			...
			FE_CSV_Import_Export_Ajax_Util::send_error_response( 'CSV parser error: ' . $t->getMessage() );
			return null;
		}
	}
	...
	private function analyze_taxonomy_format_validation_from_first_row(
		...
		foreach ( $taxonomy_format_validation as $taxonomy_name => $validation ) {
			...
			if ( ! $consistency_result['valid'] ) {
				FE_CSV_Import_Export_Ajax_Util::send_error_response( (string) $consistency_result['error'] );
				return null;
			}
		}
