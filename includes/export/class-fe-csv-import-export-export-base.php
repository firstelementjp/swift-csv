...
abstract class FE_CSV_Import_Export_Export_Base {
	...
	protected function get_taxonomy_headers( array $config ) {
		...
		$taxonomies           = apply_filters( 'fe_csv_import_export_export_filter_taxonomy_objects', $taxonomies, $taxonomy_filter_args );
	...
	protected function get_custom_field_headers( array $config, array $query_spec = [] ) {
		...
		$sample_query_args                   = apply_filters( 'fe_csv_import_export_export_sample_query_args', $sample_query_args, $sample_args );
		...
		$classified_meta_keys = apply_filters( 'fe_csv_import_export_export_classify_meta_keys', $all_meta_keys, $meta_classify_args );
		...
		$custom_field_headers = ! empty( $config['include_custom_fields'] ) ? apply_filters( 'fe_csv_import_export_export_generate_custom_field_headers', [], $classified_meta_keys, $custom_field_args ) : [];
	...
	protected function get_complete_headers( array $config, array $query_spec = [], string $context = 'standard' ) {
		 ...
		 */
		$headers = apply_filters( 'fe_csv_import_export_export_headers', $headers, $config, $context );
		do_action( 'fe_csv_import_export_export_phase_headers', $headers, $config, $context );
		return $headers;
	}
	...
	protected function get_csv_row( $post ) {
		...
		return apply_filters( 'fe_csv_import_export_export_row', $row, $post['ID'], $this->config, $scope );
	}
	...
	protected function build_csv_row_from_headers( array $post_data, array $headers ) {
		...
		foreach ( $headers as $header ) {
			...
			if ( 'ID' === $header ) {
				...
				$value       = apply_filters( 'fe_csv_import_export_export_process_custom_header', '', $header, $post_id, $custom_args );
			}
	...
	protected function validate_config( $config ) {
		...
		$validated_config = [
			...
			'taxonomy_hierarchical' => isset( $config['taxonomy_hierarchical'] ) ? (bool) $config['taxonomy_hierarchical'] : false,
