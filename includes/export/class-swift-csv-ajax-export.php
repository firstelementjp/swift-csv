...
class Swift_CSV_Ajax_Export {
	...
	public function __construct() {
		// Export execution and export log polling are handled by the unified export handler.
			...
			foreach ( $posts as $post_id ) {
				...
				foreach ( $headers as $header ) {
					...
					if ( 'ID' === $header ) {
						...
						$raw_post_fields = [ 'post_content', 'post_excerpt' ];
						if ( in_array( $header, $raw_post_fields, true ) ) {
							$value = get_post_field( $header, $post_id, 'raw' );
						} else {
