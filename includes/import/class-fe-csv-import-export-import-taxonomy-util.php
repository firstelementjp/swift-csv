	...
	public function validate_taxonomy_format_consistency(
		...
		if ( 'name' === $taxonomy_format && ! empty( $validation['all_numeric'] ) ) {
			$error = sprintf(
				...
				__( 'Format mismatch detected for taxonomy "%1$s". UI is set to "Names" but CSV contains only numeric values: %2$s. Please check your data format.', 'fe-csv-import-export' ),
		...
		if ( 'id' === $taxonomy_format && ! empty( $validation['all_string'] ) ) {
			$error = sprintf(
				/* translators: 1: taxonomy name, 2: UI format, 3: sample values */
				__( 'Format mismatch detected for taxonomy "%1$s". UI is set to "Term IDs" but CSV contains only text values: %2$s. Please check your data format.', 'fe-csv-import-export' ),
				$validation['taxonomy_name'] ?? 'unknown',
				implode( ', ', $validation['sample_values'] ?? [] )
			);
